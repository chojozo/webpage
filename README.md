...
GitHub Pages가 `main` 브랜치의 `/docs` 폴더를 바라보면 아래처럼 배포됩니다.

- `docs/afl/index.html` -> `https://chojozo.github.io/webpage/afl`
- `docs/sample/index.html` -> `https://chojozo.github.io/webpage/sample`
...
python .\scripts\build_history.py --source-type notion --output docs\afl\index.html
```
...
```powershell
$env:NOTION_TOKEN = "secret_xxx"
python .\scripts\build_history.py --source-type notion --notion-database-id "NOTION_DB_ID" --output docs\new-page\index.html
```
...
2. Branch는 `main`, folder는 `/docs`로 설정합니다.
...
워크플로가 실행되면 `docs/{slug}/index.html`을 생성하고 변경사항을 자동 커밋/푸시합니다.
